...
if __name__ == "__main__":
    ...
    # Chi2 des deux modèles oscillants (à mettre à jour si les fits changent)
    chi2_afree = 14.1725   # Script 6 : A libre par bin (4×A, 4×zeta, 4×Rc, 4×phi0)
    chi2_univ  = 15.71     # Script 8 : A universel   (1×A, 4×zeta, 4×Rc, 4×phi0)
    ...
    print("="*50)
    print(f"Chi2 Modèle Nul (A=0)       : {chi2_null:.4f}  (k={k_null})")
    print(f"Chi2 Modèle Universal A     : {chi2_univ:.4f}  (k={k_univ})")
    print(f"Chi2 Modèle A-free          : {chi2_afree:.4f}  (k={k_afree})")
    ...
    print(f"\n--- Null vs Universal A (Δk={delta_k_univ}) ---")
    print(f"  Δχ² = {delta_chi2_univ:.2f}  →  {sigma_univ:.2f} σ")

    print(f"\n--- Null vs A-free (Δk={delta_k_afree}) ---")
    print(f"  Δχ² = {delta_chi2_afree:.2f}  →  {sigma_afree:.2f} σ")

    if sigma_univ > 3:
        print("\n>>> INDICATION FORTE / DÉCOUVERTE <<<")

    # --- AIC / BIC ---
    print("\n" + "-"*50)
    print("CRITÈRES D'INFORMATION (AIC / BIC)")
    print("-"*50)
    ...
    print(f"{'Modèle':<20s} {'AIC':>8s} {'BIC':>8s}")
    ...
    print(f"\nΔAIC (Nul − Universal)   : {delta_aic_univ:.2f}  (>0 = faveur oscillant)")
    print(f"ΔBIC (Nul − Universal)   : {delta_bic_univ:.2f}  (>0 = faveur oscillant)")
    print(f"ΔAIC (A-free − Universal) : {delta_aic_afree:.2f}  (>0 = Universal préféré)")
